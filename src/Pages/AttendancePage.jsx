...
const AttendancePage = ({ showAdminAttendance, showStaffAttendance }) => {
  ...
  const handleSearchAttendanceBoxClose = () => {
    ...
    const fetchStaffData = async () => {
      ...
      await db.collection("teachers").onSnapshot((snapshot) => {
        snapshot.forEach((teacher) => {
          staffDataArray.push({ ...teacher.data(), firebaseId: teacher.id });
        });
      });
  ...
  const handleSearchAttendanceChange = (e) => {
    e.preventDefault();
    // If id is selected automatically name will be changed to correct one using filter
    if (e.target.name === "id") {
      let id = e.target.value;
      //Checking if id is of staff or admin
      if (id[0] === "a") {
        let selectedAdmin = adminData.filter((admin) => {
          return admin.aid === id;
        })[0];
        setSearchAttendanceResult({
          ...searchAttendanceResult,
        ...
        let selectedStaff = staffData.filter((staff) => {
  ...
  const handleSearchAttendanceSubmit = (e) => {
    ...
    if (searchAttendanceResult.id.match(/^a[0-9a-zA-Z]{4}$/)) {
      ...
      setIsSaving(false);
      setShowSearchResult(true);
    } else if (searchAttendanceResult.id.match(/^t[0-9a-zA-Z]{4}$/)) {
      setSearchAttendanceResult({
        ...searchAttendanceResult,
        result: getAttendanceStats(
          attendanceData.staffattendance,
      ...
      let employeeName = "";
      let staff = searchAttendanceResult.staffData.filter((staff) => {
        return staff.tid === searchAttendanceResult.id;
      })[0];
      employeeName = staff.firstname + " " + staff.lastname;
      setSearchEmplName(employeeName);
      setIsSaving(false);
  ...
  const handleStaffSubmit = async (e) => {
    ...
    for (const staff of staffData) {
      attendanceObject.attendance = {
        ...attendanceObject.attendance,
        [staff.staffid ? staff.staffid : staff.tid]: true,
      };
    }
    // Amending the attendance object
    await db
    ...
    <div className="w-100">
      ...
      {showSearchAttendanceModal ? (
        <Modal
          ...
          onClose={handleSearchAttendanceBoxClose}
                    ...
                    <div className="row">
                      <div className="col d-flex flex-column">
                        <TextField
                          ...
                          variant="standard"
                          value={moment(
                            searchAttendanceResult.month + 1,
                            "M"
                          ).format("MMMM")}
                          InputProps={{
                            readOnly: true,
                    ...
                    <div className="row">
                      ...
                      <div className="d-flex flex-column col">
                        ...
                        <TextField
                          ...
                          variant="outlined"
                          name="name"
                          value={searchAttendanceResult.name}
                          autoComplete="off"
                          onChange={(e) => {
                      ...
                      <div className="d-flex flex-column col">
                        ...
                        >
                          ...
                          {searchAttendanceResult &&
                          searchAttendanceResult.adminData
                            ? searchAttendanceResult.adminData
                                .filter((admin) => {
                                  let name =
                                  ...
                                  return name
                                    .toLowerCase()
                                    .includes(
                                      searchAttendanceResult.name
                                        ? searchAttendanceResult.name.toLowerCase()
                                        : ""
                                    );
                                })
                                .map((admin) => {
                                  return (
                                    <MenuItem value={`${admin.aid}`}>
                                      {admin.aid}
                                    </MenuItem>
                                  );
                                })
                          ...
                          {searchAttendanceResult &&
                          searchAttendanceResult.staffData
                            ? searchAttendanceResult.staffData
                                .filter((staff) => {
                                  let name =
                                ...
                                .map((staff) => {
                                  return (
                                    <MenuItem
                                      value={`${staff.staffid || staff.tid}`}
                                    >
                                      {staff.staffid || staff.tid}
                                    </MenuItem>
                                  );
                                })
                            : ""}
