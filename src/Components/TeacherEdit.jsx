...
const TeacherEdit = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (teacherData.initsalary !== teacherForm.initsalary) {
     await salaryref.doc( teacherForm.salaryfirebaseid).update({
        salary: teacherForm.initsalary,
      });
    }

    //Name if updated
    if (teacherData.firstname !== teacherForm.firstname ||teacherData.lastname !== teacherForm.lastname ) {
     await salaryref.doc( teacherForm.salaryfirebaseid).update({
    ...
    if (
      ...
      if(teacherData.studentcnicphoto.length!==0){
        await firebase
        .storage()
    ...
    if (
      ...
      if(teacherData.degreefile.length!==0){
        await firebase
        .storage()
