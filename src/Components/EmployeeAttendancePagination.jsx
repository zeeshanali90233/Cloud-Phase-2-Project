...
const EmployeeAttendancePagination = ({
  ...
  const handleAttendanceCheckbox = (e, index, id) => {
    e.preventDefault();
    let updatedAttendance = [...attendanceDataPagination];
    updatedAttendance[index].attendance[id] =
      !updatedAttendance[index].attendance[id];
                ...
                .map((row, index) => {
                  return true ? (
                    <StyledTableRow key={row.firebaseId}>
                      ...
                      row.lastname +
                      "(" +
                      (isAdminAttendance ? row.aid : row.staffid || row.tid) +
                      ")"}
                      </StyledTableCell>
                        ...
                        return index === 0 ? (
                          <StyledTableCell align="center">
                            <Checkbox
                              ...
                              checked={
                                attendance.attendance[row.aid || row.staffid || row.tid]
                              }
                            />
                          </StyledTableCell>
                        ) : (
