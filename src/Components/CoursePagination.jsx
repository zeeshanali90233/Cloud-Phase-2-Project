...
const CoursePagination = ({
  ...
  const [confirmIdentityForm, setConfirmIdentityForm] = useState({
    password: "",
  });
  ...
  const handleConfirmIdentityClose = () => {
    setShowConfirmIdentityBox(false);
    setConfirmIdentityForm({ password: "" });
    setCourseToDelete("");
    setPasswordErrorMsg("");
  };
  ...
  const handleCourseDelete = async (e) => {
    ...
    if (
      window.confirm(
        "Are you sure to delete this course because all related material will be deleted?"
      )
    ) {
      try {
  ...
  return (
    <TableContainer component={Paper} className="mt-1">
      ...
      <Table sx={{ minWidth: 700 }} aria-label="customized table">
        <TableHead>
          <TableRow>
            <StyledTableCell className="text-center">Classes</StyledTableCell>
          </TableRow>
        </TableHead>
        <TableBody
        ...
        >
          {(rowsPerPage > 0
            ...
            return row.courseName
              ...
              row.courseId.toLowerCase().includes(search.toLowerCase()) ||
              search.length === 0 ? (
              <StyledTableRow key={row.courseId}>
                ...
                <Card sx={{ maxWidth: 300 }}>
                  <CardActionArea
                    onClick={() => {
                      handleNavigation(row.firebaseId);
                    }}
                  >
                    <CardMedia
                      component="img"
                    ...
                    <CardActions>
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          setShowConfirmIdentityBox(true);
                        }}
                      >
                        Delete
                      </button>
          ...
          <Box sx={style}>
            ...
            >
              ...
              {passwordErrorMsg.length !== 0 ? (
                <span className="required">{passwordErrorMsg}</span>
              ) : (
                ""
              )}
