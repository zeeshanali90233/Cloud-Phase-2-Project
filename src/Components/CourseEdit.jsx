...
const CourseEdit = ({ course }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      courseForm.courseId !== courseData.courseId ||
      courseForm.courseName !== courseData.courseName ||
      courseForm.courseDuration !== courseData.courseDuration ||
      courseForm.courseInstructorId !== courseData.courseInstructorId ||
      courseForm.noOfStudents !== courseData.noOfStudents ||
      courseForm.courseDesc !== courseData.courseDesc
    ) {
    ...
    if (
      ...
      if (courseData.courseInstructorId !== "") {
        db.collection("teachers")
          .doc(courseData.courseInstructorId)
          .update({
            courses: firebase.firestore.FieldValue.arrayRemove(
              courseData.firebaseId
            ),
    ...
    if (courseForm.courseName !== courseData.courseName) {
      ...
      if (courseForm.noOfStudents !== 0) {
        ...
        // Fees side update
        courseForm.studentsfee.forEach((feeId) => {
    ...
    const { courseThumbnail, courseAdditionalDocs, courseOutlineFile } =
      courseForm;
    if (
      JSON.stringify(courseForm.courseThumbnail) !==
      JSON.stringify(courseData.courseThumbnail)
    ) {
      await firebase
        .storage()
        .refFromURL(courseData.courseThumbnail.URL)
        .delete()
        .then(async () => {
          const metadata = {
    ...
    if (
      JSON.stringify(courseForm.courseOutlineFile) !==
      ...
      if (courseData.courseOutlineFile.length !== 0) {
        await firebase
          .storage()
          .refFromURL(courseData.courseOutlineFile.courseOutlineFileURL)
          .delete()
          .then(async () => {
    ...
    if (
      courseForm.courseAdditionalDocs.length !== 0 &&
      courseForm.courseAdditionalDocs[0].URL === undefined &&
      ...
      if (
        courseData.courseAdditionalDocs &&
        courseData.courseAdditionalDocs.length !== 0
      ) {
        ...
        <div className=" d-flex justify-content-center me-auto">
          <div>
            {courseThumbnailPic ? (
              <img
                src={
                  courseThumbnailPic.URL !== undefined
                    ? courseThumbnailPic.URL
                    : courseThumbnailPic
                }
                className="rounded"
        ...
        <div className="row">
          ...
          <div className="d-flex flex-column mt-2">
            <TextField
              ...
              placeholder="Enter the total lectures of course"
