...
const StaffEdit = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (staffData.initsalary !== staffForm.initsalary) {
      await db.collection("salaries").doc(staffForm.salaryfirebaseid).update({
        salary: staffForm.initsalary,
      });
    }
    // If name changes
    if (
      staffData.firstname !== staffForm.firstname ||
      staffData.lastname !== staffForm.lastname
    ) {
      await db
        .collection("salaries")
        .doc(staffForm.salaryfirebaseid)
        .update({
    ...
    if (
      ...
      if (staffData.staffcnicphoto.length !== 0) {
        await firebase
          .storage()
    ...
    if (
      ...
      for (var i = 0; i < staffForm.additionaldocuments.length; i++) {
        ...
        const additionaldocumentSnapshot = await storageRef
          .child(
            `staff/${staffForm.staffid}/additionaldocuments${Number(i) + 1}`
          )
      ...
      >
        ...
        <div className=" d-flex justify-content-center me-auto">
          <div>
            <img
              src={
                staffPhotoURL === "" ? staffForm.staffphoto.URL : staffPhotoURL
              }
              alt="Staff "
              width={130}
            />
