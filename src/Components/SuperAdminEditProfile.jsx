...
const SuperAdminEditProfile = () => {
  ...
  const [superAdminPhotoURL, setSuperAdminPhotoURL] = useState(
    superAdminForm && superAdminForm.superadminphoto !== undefined
      ? superAdminForm.superadminphoto.URL
      : ""
  );
  ...
  const handleSubmit = async (e) => {
    ...
    // Profile Photo
    if (superAdminForm.superadminphoto.URL === undefined) {
      const metadata = {
    ...
    //CNIC Photo
    if (superAdminForm.superadmincnicphoto.URL === undefined) {
      const metadata = {
    ...
    if (superAdminForm.medicalrecordsphoto.URL === undefined) {
      const metadata = {
    ...
    //Degree File
    if (superAdminForm.degreefile.URL == undefined) {
      const metadata = {
    ...
    //CV
    if (superAdminForm.cv.URL === undefined) {
      const metadata = {
        contentType: cv.type,
        customMetadata: {
    ...
    if (
      superAdminForm.additionaldocuments.length !== 0 &&
      superAdminForm.additionaldocuments[0].URL === undefined
    ) {
      let metadata = {};
  ...
  return (
    <div className="add-admin-form w-100 ">
      ...
      {!isObjectEmpty(superAdminForm) ? (
        <div
          className="files mt-2 w-100 d-flex container flex-wrap justify-content-evenly"
          style={{ rowGap: "12px" }}
        >
          {/* Profile Photo */}
          {superAdminForm && superAdminForm.superadminphoto !== "" ? (
            <button
              className="panel border profilephoto d-flex  px-4 py-3 align-items-center rounded justify-content-center"
              onClick={() => {
                handleDownload(
                  superAdminData.superadminphoto,
                  `${
                    superAdminData.firstname.toUpperCase() +
                    " " +
                    superAdminData.lastname.toUpperCase() +
                    "(" +
                    superAdminData.said +
                    ")"
                  }`
          ...
          {/* CV*/}
          {superAdminForm && superAdminForm.cv.length !== 0 ? (
            <button
              className="panel border parentcnic d-flex  px-4 py-3 align-items-center rounded justify-content-center"
              onClick={() => {
                handleDownload(
