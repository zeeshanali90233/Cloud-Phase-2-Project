...
const AdminEdit = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    await adminRef.doc(adminForm.firebaseId).update({
      ...
      canmanagesalary: Boolean(adminForm.canmanagesalary),
    });

    // If salary changes than also updating it in the salary collection
    if (adminData.initsalary !== adminForm.initsalary) {
      await  db.collection("salaries").doc(adminData.salaryref).update({
        salary: adminForm.initsalary,
      });
    }

    // If name changes
    if (adminData.firstname !== adminForm.firstname || adminData.lastname !== adminForm.lastname) {
      await  db.collection("salaries").doc(adminData.salaryref).update({
