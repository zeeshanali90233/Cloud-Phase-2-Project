...
const AdminEditProfile = () => {
  ...
  const [adminPhotoURL, setAdminPhotoURL] = useState(
    adminForm && adminForm.adminphoto !== undefined
      ? adminForm.adminphoto.URL
      : ""
  );
  ...
  const handleSubmit = async (e) => {
    ...
    // Profile Photo
    if (
      adminForm.adminphoto.URL === undefined &&
      adminForm.adminphoto !== adminData.adminphoto
    ) {
      const metadata = {
      ...
      if (adminData.adminphoto.length !== 0) {
        await firebase
          ...
          .then(async () => {
            ...
            // Updating the profile in salary receipt also
            await db.collection("salaries").doc(adminForm.salaryref).update({
    ...
    // CNIC Photo
    if (adminForm.admincnicphoto.URL === undefined) {
      const metadata = {
    ...
    // Medical Records
    if (adminForm.medicalrecordsphoto.URL === undefined) {
      const metadata = {
    ...
    // Degree
    if (adminForm.degreefile.URL === undefined) {
      const metadata = {
    ...
    if (
      adminForm.additionaldocuments.length !== 0 &&
      adminForm.additionaldocuments[0].URL === undefined
    ) {
      let metadata = {};
      //Deleting the previous additional Files
      if (
        adminData.additionaldocuments &&
        adminData.additionaldocuments.length !== 0
      ) {
    ...
    <div className="add-admin-form container mt-2 w-100 ">
      ...
      {!isObjectEmpty(adminForm) ? (
        <div className="files mt-2 w-100 d-flex flex-wrap justify-content-evenly">
          ...
          {/* Additional Records */}
          {adminForm && adminForm.additionaldocuments.length !== 0 ? (
            <button
          ...
          >
            ...
            <div className=" d-flex justify-content-center me-auto">
              <div>
                <img
                  className="rounded"
                  src={adminPhotoURL !== "" ? adminPhotoURL : emptyProfile}
                  alt="AdminPhoto "
                  width={130}
                />
              </div>
